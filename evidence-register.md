...
| `interconnect.gpu-pair-direction` | 150 | GB/s | one direction between a Booster node GPU pair | `official` | high | [`jsc-config`](https://apps.fz-juelich.de/jsc/hps/jupiter/configuration.html) |  |
| `interconnect.cpu-pair-direction` | 100 | GB/s | one direction between a Booster node Grace CPU pair | `official` | medium | [`jsc-config`](https://apps.fz-juelich.de/jsc/hps/jupiter/configuration.html), [`jsc-tech`](https://www.fz-juelich.de/en/jsc/jupiter/tech/) | The current configuration page states 100 GB/s per direction; wording differs across JSC summaries, so this value is not used as a measured application-throughput claim. |
...
| `network.hca-link` | 200 | Gbit/s | one Booster node ConnectX-7 HCA to L1 | `official` | high | [`jsc-config`](https://apps.fz-juelich.de/jsc/hps/jupiter/configuration.html) |  |
...
| `network.bandwidth-node-bytes` | 100 | GB/s | Booster node decimal line-rate equivalent | `derived` | high | [`jsc-config`](https://apps.fz-juelich.de/jsc/hps/jupiter/configuration.html) | 800 Gbit/s divided by 8; protocol and application throughput are lower. |
| `network.switch-link` | 400 | Gbit/s | one DragonFly+ switch-to-switch link | `official` | high | [`jsc-config`](https://apps.fz-juelich.de/jsc/hps/jupiter/configuration.html) |  |
| `network.switch-link-bytes` | 50 | GB/s | one DragonFly+ switch-to-switch link decimal equivalent | `derived` | high | [`jsc-config`](https://apps.fz-juelich.de/jsc/hps/jupiter/configuration.html) | 400 Gbit/s divided by 8; this is a link rate, not rack-to-rack throughput. |
| `network.l1-switches-group` | 15 | switches | one full Booster DragonFly+ group | `official` | high | [`jsc-config`](https://apps.fz-juelich.de/jsc/hps/jupiter/configuration.html) |  |
| `network.l2-switches-group` | 16 | switches | one full Booster DragonFly+ group | `official` | high | [`jsc-config`](https://apps.fz-juelich.de/jsc/hps/jupiter/configuration.html) |  |
...
| `topology.dragonfly-groups-total` | 27 | groups | common JUPITER fabric: Booster, Cluster and administrative systems | `official` | high | [`jsc-config`](https://apps.fz-juelich.de/jsc/hps/jupiter/configuration.html) | 25 Booster groups, one Cluster group and one administrative group. |
